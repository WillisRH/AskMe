...
async function checkAuthServer() {
  try {
    ...
    if (response.status === 200) {
      console.log(
        "\x1b[32m" + "[AuthAPI] Authentication server is running." + "\x1b[37m"
      );
    } else {
      console.log(
        "\x1b[31m" +
          "[AuthAPI] Authentication server is not running." +
          "\x1b[37m"
      );
      mt = true;
    }
...
app.post("/home/askme/admin/signupatt", async (req, res) => {
  ...
  try {
    ...
    if (response.status == "400") {
      ...
      return console.log(
        "The Username is already Exist! Please try again! (400)"
      );
    }
    if (!response.ok) {
    ...
    console.log(
      "Success logged in with username '" +
        email +
...
app.post("/home/askme/admin/loginatt", async (req, res, next) => {
  ...
  try {
    ...
    if (response.status == "400") {
      res.render("loginhandler.ejs", {
        error: "Your password or username is incorrect. Please try again!",
      });
      return console.log(
...
app.get("/deletecookies", async (req, res) => {
  ...
  res.render("loginhandler.ejs", {
    error: "Successfully deleting the cookies!",
  });
});
